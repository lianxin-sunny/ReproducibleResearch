...
Here we use plyr package to do most of the data cleaning job.
...
Here we use the avarege of that interval to imput as the missing data.
...
Mean values didn't change, because here we supplied interval averge for the given interval, so total average didn't change. Histogram and median changed somewhat. Filling values added total of 8.613 &times; 10<sup>4</sup> steps.
...
data_with_weekdays$wk <- ifelse(data_with_weekdays$day %in% c("ÐÇÆÚÁù", "ÐÇÆÚÈÕ"), 
    "weekend", "weekday")
# ATTENTION: My laptop has a Chinese environment, so I use Chinese to split
